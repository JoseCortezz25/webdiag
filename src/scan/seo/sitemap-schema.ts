/**
 * The sitemaps.org 0.9 schemas, vendored verbatim.
 *
 * They are embedded as strings rather than shipped as files for two reasons.
 * `bun build` bundles a single `dist/cli.js`, so a sibling `.xsd` would simply
 * not be there in an installed CLI; and fetching them from sitemaps.org at scan
 * time would make the validity of a client's sitemap depend on whether that
 * host was up, which is not a property of the client's sitemap. `xmllint` is
 * invoked with `--nonet` so this really is the only copy in play.
 *
 * Retrieved from https://www.sitemaps.org/schemas/sitemap/0.9/ (last modified
 * 2008-03-26 upstream). Neither file imports another schema, so no rewriting of
 * `schemaLocation` is needed.
 */

/** Validates a `<urlset>` document. */
export const SITEMAP_XSD = `<?xml version="1.0" encoding="UTF-8"?>
...
/** Validates a `<sitemapindex>` document. */
export const SITEINDEX_XSD = `<?xml version="1.0" encoding="UTF-8"?>
<xsd:schema xmlns:xsd="http://www.w3.org/2001/XMLSchema"
            targetNamespace="http://www.sitemaps.org/schemas/sitemap/0.9"
            xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
            elementFormDefault="qualified">
